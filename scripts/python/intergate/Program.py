...
class Program():
    ...
    def __do_run_eoos_ut(self, config):
        ...
        ret = subprocess.run([self.__args.interpreter, 'Build.py', '-c', '-b', 'ALL', '-r', '--config', config, '-j', '8']).returncode
    ...
    def __do_install_eoos(self, config):
        ...
        os.chdir(f'{self.__get_path_to_eoos_dir()}/scripts/python')
        ret = subprocess.run([self.__args.interpreter, 'Build.py', '-c', '-b', 'EOOS', '--install', '--config', config, '-j', '8']).returncode
        os.chdir(self.__PATH_FROM_A_PROJECT_DIR)
        if ret != 0:
    ...
    def __do_run_eoos_sample_applications(self, config):
        ...
        ret = subprocess.run([self.__args.interpreter, 'Build.py', '-c', '-b', '-r', '--config', config, '-j', '8']).returncode
        os.chdir(self.__PATH_FROM_A_PROJECT_DIR)
        if ret != 0:
